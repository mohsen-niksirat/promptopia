...
async function cmdBuild({ force = false } = {}) {
  ...
  /* atomic write — Defender/indexer briefly locks freshly-written files on Windows */
  const dataTmp = path.join(DATA, 'prompts.js.tmp');
  fs.writeFileSync(
    dataTmp,
  ...
  fs.renameSync(dataTmp, path.join(DATA, 'prompts.js'));
